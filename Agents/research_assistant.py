...
3. Determine the most interesting themes based upon the topic.
...
def search_wikipedia(state: InterviewState):
    ...
    try:
        ...
        try:
            ...
            # Return empty context if Wikipedia fails
            return {"context": []}
        ...
        print(f"Error in search_wikipedia: {str(e)}")
...
def human_feedback(state: GenerateAnalystsState):
    """ Auto-approve the analysts and return the next state """
    return {"human_analyst_feedback": "approve", "next": "conduct_interview"}
...
# Entry point function for supervisor agent
async def run_research_agent(state: dict) -> dict:
    # Extract topic from the first user message if not provided
    if "topic" not in state:
        if "messages" in state and state["messages"]:
            # Get the last user message
            for msg in reversed(state["messages"]):
                if msg.get("role") == "user":
                    state["topic"] = msg.get("content", "Research topic not specified")
                    break
        else:
            state["topic"] = state.get("input", "Research topic not specified")
    
    # Set default values for research-specific fields
    state["max_analysts"] = state.get("max_analysts", 3)
    state["analysts"] = state.get("analysts", [])
    state["sections"] = state.get("sections", [])
    state["introduction"] = state.get("introduction", "")
    state["content"] = state.get("content", "")
    state["conclusion"] = state.get("conclusion", "")
    ...
    return {
        "research_output": result.get("final_report", ""),
        "last_ran_agent": "research"
    }